...
class Repuesto(models.Model):
    ...
    cantidad_bodega=models.PositiveSmallIntegerField(default=0)
    foto=models.ImageField(upload_to="images_repuestos/",null=True,blank=True)
    anexo=models.FileField(upload_to='archivos_repuestos/', blank=True, null=True)

    def save_foto(self, *args, **kwargs):
        if self.foto:
            extension = self.foto.name.split('.')[-1]
            new_filename = f"{self.id}_{self.nombre}.{extension}"
    ...
    def save_link(self):
        ...
        if self.anexo:
            ...
            if self.anexo.name.endswith('.pdf'):
                ...
                new_file_name = f"{self.id}_{self.nombre}.pdf"
