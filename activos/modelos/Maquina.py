...
class Maquina(models.Model):        
    ...
    area=models.ForeignKey(SubArea,on_delete=models.CASCADE)                
    num_maquina=models.IntegerField()
    codigo=models.CharField(max_length=25,null=True,blank=True)             
    nombre=models.CharField(max_length=100)                                 
    descripcion=models.TextField()                                                     
    disponibilidad=models.CharField(max_length=3,choices=estado_operativo)  
    fecha_instalacion = models.DateField()                                  
    fecha_updated=models.DateField(auto_now=True)                           
    foto=models.ImageField(upload_to="images_maquinas/",null=True,blank=True) 
    link=models.FileField(upload_to="archivos_maquinas/",null=True,blank=True) 

    def save_foto(self, *args, **kwargs):
        if self.foto:
            extension = self.foto.name.split('.')[-1]
            new_filename = f"{self.codigo}_{self.nombre}.{extension}"
    ...
    def save_link(self):
        ...
        if self.link:
            ...
            if self.link.name.endswith('.pdf'):
                with self.link.open('rb') as f:
                    compressed_pdf = compress_pdf(f)
                
                # Crear un nuevo nombre para el archivo PDF basado en 'codigo' y 'nombre'
                new_file_name = f"{self.codigo}_{self.nombre}.pdf"
                new_file_path = f"{new_file_name}"

                # Eliminar el archivo anterior
                self.link.delete(save=False)  # Eliminar el archivo anterior

                # Guardar el archivo comprimido con el nuevo nombre
                self.link = ContentFile(compressed_pdf.read(), name=new_file_path)
